...
export default function Home() {
  ...
  const onChangeFirstInput = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
    setFirstInput(event.target.value)
  }

  const onChangeFollowupInput = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      return (
        <div className="flex items-center w-full max-w-3xl p-5 sm:px-7 gap-4 border-t-2 border-tertiary bg-tertiary">
          ...
          {loading ? (
            ...
            <SanitizeHTML className="flex-1 overflow-hidden leading-6 ai-answer" html={chatHtml} />
          )}
        </div>
      )
    }
      ...
      <main className="mx-auto bg-primaryLight min-h-screen h-full flex flex-col items-center  py-8 md:py-12">
        ...
        {previousMessages.map(({ htmlContent, isUser, key }) => (
          ...
          >
            ...
            {isUser ? (
              <div className="flex-1 overflow-hidden leading-6 ai-answer">{htmlContent}</div>
            ) : (
              <SanitizeHTML className="flex-1 overflow-hidden leading-6 ai-answer" html={htmlContent} />
            )}
          </div>
        ))}
        {getChatContent()}
